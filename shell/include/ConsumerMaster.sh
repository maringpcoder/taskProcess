...
function ConsumerMaster
{
     ...
     inifile=$2
     main_path=$3
     theProcessFilePath="${main_path}ConsumerMaster.php"
     process_prefix=`readIni ${inifile} "log_section" "panda_process"`
     process_master=`readIni ${inifile} "log_section" "log_master_name"`
     process_worker=`readIni ${inifile} "log_section" "log_kafka"`
     worker_num=`readIni ${inifile} "log_section" "worker_num"`
     ...
     "start")
             ...
             then
                ...
                then
                    ...
                    #重启
                    nohup ${phpcmd} ${theProcessFilePath} >> ConsumerMaster.log &
                    Echo_Green "重启..............OK"
                else
                    echo "nothing to do !"
                    exit 1;
